...
class Model
{
	...
	/**
	 * Set this model's additional conditions.
	 *
	 * A necessary condition for every time select data.
	 *
	 * @var array
	 */
	static $append_conditions;
	 ...
	 * @var array
	 */
	static $VALID_OPTIONS = array('enable_append_conditions', 'conditions', 'limit', 'offset', 'order', 'select', 'joins', 'include', 'readonly', 'group', 'from', 'having');
	...
	public static function count(/* ... */)
	{
		...
		/* Modify by OA at 2014-02-14 */
		if (isset($options['select']) && is_string ($options['select']) && (strpos (strtoupper ($options['select']), 'DISTINCT') !== false)) $options['select'] = 'COUNT(' . $options['select'] . ')';
		else $options['select'] = 'COUNT(*)';
		...
		if (isset ($options['enable_append_conditions'])) unset ($options['enable_append_conditions']);
