Welcome to MYWALL!
  這是一個分享圖片的社群，主要是參考了 STYLEWALL ，將 缺點改進 並且 加強效能 的簡單版本。


聲明:
  本作品只限分享於研究、研討性質之使用，並不提供任何有營利效益之使用。 如有營利用途，務必告知作者 OA(comdan66@gmail.com)，並且經由作者同意。


特別注意:
  下列步驟中，由於在初始網站時候會建立初始資料以便於 Demo 或者研究函式功能，建立方法是藉由爬取 STYLEWALL 網站的首頁資源建立，如有進一步相關應用與授權，請與 STYLEWALL 官方聯絡，如有違法之行為，作者不負相關責任。
...
快速安裝:
  ...
  4. 修改 application/config/database.php 資料，建議 $db['default']['database'] 值改成 mywall 然後輸入你的 MySQL 帳號、密碼，並且在你的 MySQL 建立一張表，建議名稱叫做 mywall。

  5. 初始網站，開啓瀏覽器，並網址輸入 :
      http:// your domain /demo/c
      開啓網頁後，程式會自動至 STYLEWALL 爬取資料，所以會有點久。
...
一般安裝:
  ...
  4.然後修改 database.php 資料，建議 $db['default']['database'] 值改成 mywall 然後輸入你的 MySQL 帳號、密碼，並且在你的 MySQL 建立一張表，建議名稱叫做 mywall。
