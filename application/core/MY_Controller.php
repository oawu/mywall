...
class Root_controller extends CI_controller {
  ...
  protected function init_component_lists () {
    ...
    if (count ($components = func_get_args ()))
...
class Site_controller extends Root_controller {
  public function __construct () {
    ...
    $this->set_componemt_path ('component', 'site')
         ->set_frame_path ('component', 'frame')
         ->set_content_path ('component', 'content')
         ->set_title ('123')
         ->init_component_lists ('javascript', 'css', 'hidden', 'meta')
         ->add_meta (array ('http-equiv' => "Content-type", "content" => "text/html; charset=utf-8"))
         ->add_hidden (array ('id' => 'asasd', 'value' => 'dasd'))
         ->add_css ('sada', 'dqweds', 'dasd')
         ->add_javascript ('sada', 'dqweds', 'dasd');

         // echo '<meta http-equiv="Content-type" content="text/html; charset=utf-8" /><pre>';
         // var_dump ($this->get_component_lists ());
