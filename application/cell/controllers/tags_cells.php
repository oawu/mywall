...
class Tags_cells extends Cell_Controller {
  ...
  public function pictures ($next_id = 0, $tags = array ()) {
    ...
    $pics = Picture::find ('all', array ('order' => 'year_week DESC, like_count DESC', 'include' => array ('user'), 'conditions' => array ('id IN (?)', array_slice ($picture_ids, 0, config ('tags_controller_config', 'pictures_length')))));
    $pictures = array ();
