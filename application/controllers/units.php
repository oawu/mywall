...
class Units extends Site_controller {
  ...
  public function tags ($names = '') {
    if (!count ($names = array_filter (explode (' ', rawurldecode ($names)))) || !count ($tags = UnitTag::find ('all', array ('order' => 'units_count DESC', 'conditions' => array ('name in (?)', $names))))) redirect (array ($this->get_class (), 'index'));
  ...
  public function id ($id_or_name) {
    ...
    $this->set_title ($unit->name . ' - 北港地圖故事')
         ...
         ->add_javascript ('https://maps.googleapis.com/maps/api/js?v=3.exp&sensor=false&language=zh-TW')
         
         ->add_hidden ('submit_comment_url', 'submit_comment_url', base_url (array ($this->get_class (), 'submit_comment')))
         ->add_hidden ('read_more_comment_url', 'read_more_comment_url', base_url (array ($this->get_class (), 'read_more_comment')))
         ->add_hidden ('set_score_url', 'set_score_url', base_url (array ($this->get_class (), 'set_score')))
         ->add_hidden ('delete_unit_url', 'delete_unit_url', base_url (array ($this->get_class (), 'delete_unit')))
         ->add_hidden ('delete_unit_comment_url', 'delete_unit_comment_url', base_url (array ($this->get_class (), 'delete_unit_comment')))
         ->add_hidden ('advice_unit_url', 'advice_unit_url', base_url (array ($this->get_class (), 'advice_unit')))
         ->add_hidden ('push_advice_message_url', 'push_advice_message_url', base_url (array ($this->get_class (), 'push_advice_message')))
         ->add_hidden ('fb_sing_in_url', 'fb_sing_in_url', $this->fb->getLoginUrl (array ('redirect_uri' => base_url (array ('platform', 'fb_sing_in', implode ('|', array ($this->get_class (), 'id', $unit->id)))))))
         ->add_hidden ('fetch_star_details_url', 'fetch_star_details_url', base_url (array ($this->get_class (), 'fetch_star_details')));
      
      $comments = render_cell ('unit_cells', 'comments', $unit);
      ...
      $this->set_method ('id')->load_view (array ('unit' => $unit, 'view' => $unit->view, 'tags' => $unit->tags, 'more_tags' => $more_tags, 'banner_type' => $banner_type, 'comment_list' => $comments['comment_list'], 'pictures' => $pictures, 'next_id' => $comments['next_id']));
  ...
  public function read_more_comment () {
    ...
    if (verifyNumber ($unit_id) && verifyNumber ($next_id) && verifyObject ($unit = Unit::find ('one', array ('conditions' => array ('id = ? AND status = ?', $unit_id, 'certified'))))) {
      $comments = render_cell ('unit_cells', 'comments', $unit, $next_id);
      $this->output_json (array ('status' => true, 'next_id' => $comments['next_id'], 'contents' => $comments['comments']));
    } else { redirect (array ('error', 'not_ajax')); }
  }
  ...
  public function fetch_star_details () {
    ...
    if (verifyNumber ($unit_id, 1) && verifyObject ($unit = Unit::find ('one', array ('conditions' => array ('id = ? AND status = ?', $unit_id, 'certified'))))) {
      clear_cell ('unit_cells', 'star_details', $unit->id);
      clear_cell ('unit_cells', 'score_star', $unit->id);
      $this->output_json (array ('status' => true, 'score_star' => render_cell ('unit_cells', 'score_star', $unit), 'star_details' => render_cell ('unit_cells', 'star_details', $unit)));
    } else { $this->output_json (array ('status' => true, 'message' => '參數資訊有誤，請稍候再試，或請通知程式設計人員!')); }
  }
  ...
  public function set_score () {
    ...
    if (verifyNumber ($unit_id, 1) && verifyNumber ($score, 1, 5)) {
      if (identity ()->get_identity ('sign_in') && verifyNumber ($user_id = identity ()->get_session ('user_id'), 1) && verifyObject ($user = User::find ('one', array ('select' => 'id', 'conditions' => array ('id = ?', $user_id))))) {
        if (verifyObject ($unit = Unit::find ('one', array ('select' => 'id', 'conditions' => array ('id = ? AND status = ?', $unit_id, 'certified'))))) {
          ...
          } else { $this->output_json (array ('status' => false, 'title' => '提示', 'message' => '你應該已經評過分數囉，如有任何問題請通知管理人員!', 'action' => 'function () { $(this).OA_Dialog ("close"); }')); }
        } else { $this->output_json (array ('status' => false, 'title' => '失敗', 'message' => '評分失敗，此景點暫時不提供評分，如有任何問題請通知管理人員!', 'action' => 'function () { $(this).OA_Dialog ("close"); }')); }
      } else { $this->output_json (array ('status' => false, 'title' => '提示', 'message' => '你還沒登入喔！趕快按下確定，就可以輕鬆使用 Facebook 登入評分囉!', 'action' => 'function () { window.location.assign ("' . $this->fb->getLoginUrl (array ('redirect_uri' => base_url (array ('platform', 'fb_sing_in', implode ('|', array ($this->get_class (), 'id', $unit_id)))))) . '"); }')); }
    } else { $this->output_json (array ('status' => false, 'title' => '錯誤', 'message' => '評分錯誤，請稍候再試，或請通知程式設計人員!', 'action' => 'function () { $(this).OA_Dialog ("close"); }')); }
  }
